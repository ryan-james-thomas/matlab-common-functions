...
function m = read_labview_binary(fname,dim,varargin)
...
m=zeros(512,512,size(data2,1));
for nn=1:size(data2,1),
    m(:,:,nn)=flipud(reshape(data2(nn,:),512,512));
end;

% ImageAtoms=reshape(data2(1,:),512,512);
% ImageNoAtoms=reshape(data2(2,:),512,512);
% ImageDark=reshape(data2(3,:),512,512);
% 
% m(:,:,1)=flipud(ImageAtoms);
% m(:,:,2)=flipud(ImageNoAtoms);
% m(:,:,3)=flipud(ImageDark);
